...
BEGIN
								...
								case when @Type='agency' 
										then
											case when @Value = 'health@@@other' 
												...
												when @Value = 'police@@@fire' 
													then ' and rtrim(isnull(sub.AgencyName,''Miscellaneous'')) in (''police'',''fire'') '														
	...
	SET @SQL = @SQL + case when @Psychological_Claims <> 'all' then ' and [Is_Stress] = 1' else '' end
	SET @SQL = @SQL + case when @Inactive_Claims <> 'all' then ' and [Is_Inactive_Claims] = 1' else '' end
	SET @SQL = @SQL + case when @Medically_Discharged <> 'all' then ' and [Is_Medically_Discharged] = 1' else '' end
	SET @SQL = @SQL + case when @Exempt_From_Reform <> 'all' then ' and [Is_Exempt] = 1' else '' end
	SET @SQL = @SQL + case when @Reactivation <> 'all' then ' and [Is_Reactive] = 1' else '' end
	...
	BEGIN
		...
		SET @SQL = 'SELECT Claim_No, Claim_Closed_Flag ' +
					...
					' WHERE ISNULL(Is_Last_Month,0) = 0 ' + @WHERE_CONS +
	...
	BEGIN
		...
		(
			SELECT DISTINCT AgencyName
				FROM #claim_list
				WHERE AgencyName <> '' and Claim_Closed_Flag <> 'Y'
				GROUP BY AgencyName
				HAVING COUNT(*) > 0
		) as tmp_value
		--End Agency--
		...
		(
			SELECT DISTINCT [Group]
				FROM #claim_list
				WHERE [Group] <> '' and Claim_Closed_Flag <> 'Y'
				GROUP BY [Group]
				HAVING COUNT(*) > 0
		) as tmp_value
		--End Group--
		...
		(
			SELECT DISTINCT AgencyName, Sub_Category
				FROM #claim_list
				WHERE AgencyName <> '' and Sub_Category <> '' and Claim_Closed_Flag <> 'Y'
		...
		(
			SELECT DISTINCT [Group], Team
				FROM #claim_list
				WHERE [Group] <> '' and Team <> '' and Claim_Closed_Flag <> 'Y'
				GROUP BY [Group], Team
				HAVING COUNT(*) > 0
		) as tmp_value
		--End Team--
		...
		(
			SELECT DISTINCT Team, Claims_Officer_Name
				FROM #claim_list
				WHERE Team <> '' and Claims_Officer_Name <> '' and Claim_Closed_Flag <> 'Y'
	...
	BEGIN
		...
		(
			SELECT DISTINCT EMPL_SIZE
				FROM #claim_list
				WHERE EMPL_SIZE <> '' and Claim_Closed_Flag <> 'Y'
				GROUP BY EMPL_SIZE
				HAVING COUNT(*) > 0
		) as tmp_value
		--End Employer size--
		...
		(
			SELECT DISTINCT Account_Manager
				FROM #claim_list
				WHERE Account_Manager <> '' and Claim_Closed_Flag <> 'Y'
				GROUP BY Account_Manager
				HAVING COUNT(*) > 0
		) as tmp_value
		--End Account manager--
		...
		(
			SELECT DISTINCT Account_Manager, EMPL_SIZE
				FROM #claim_list
				WHERE Account_Manager <> '' and EMPL_SIZE <> '' and Claim_Closed_Flag <> 'Y'
	...
	BEGIN
		...
		(
			SELECT DISTINCT Portfolio
				FROM #claim_list
				WHERE Portfolio <> '' and Claim_Closed_Flag <> 'Y'
				GROUP BY Portfolio
				HAVING COUNT(*) > 0
		) as tmp_value
		--End Portfolio--
		...
		(
			SELECT DISTINCT Portfolio, EMPL_SIZE
				FROM #claim_list
				WHERE Portfolio <> '' and EMPL_SIZE <> '' and Claim_Closed_Flag <> 'Y'
...
BEGIN
	...
	SET @SQL = 'SELECT *				
				...
				+							 
				(case when UPPER(@System) = 'TMF'
						then case when @Value <> 'all' and @Value <> 'health@@@other' and @Value <> 'police@@@fire' and @Type = 'agency' then ' and [Agency_Name] = ''' + @Value + ''''
								when @Value <> 'all' and @Type = 'group' then ' and [Group] = ''' + @Value	+ ''''						  
								when @Value = 'health@@@other' then ' and [Agency_Name] in (''health'',''other'')' 
								when @Value = 'police@@@fire' then ' and [Agency_Name] in (''police'',''fire'') '
								when @Value = 'all' then ''
								else ''
							end
					else ''
	...
	SET @SQL = @SQL + (case when @ClaimType like '%med_only'
							...
							when @ClaimType like '%ncmm_this_week'
								 then ' and [NCMM_Actions_This_Week] <> '''' '
...
BEGIN
	...
	SET @SQL = 'SELECT Value=' + case when UPPER(@System) = 'TMF'
				...
				,[Is_Reactive],[NCMM_Actions_This_Week],[NCMM_Actions_Next_Week]
	...
	BEGIN
		...
		BEGIN
			...
			-- Grouping Value: Police & Fire	
			INSERT INTO #total
			SELECT Value = 'POLICE & FIRE', Claim_Type, iClaim_Type, SUM(overall) as overall
			FROM #total
			WHERE Value = 'Police' or Value = 'Fire'
			GROUP BY Claim_Type, iClaim_Type
		END
		ELSE IF UPPER(@System) = 'EML'
		BEGIN
