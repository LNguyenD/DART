...
BEGIN
	...
	IF OBJECT_ID('tempdb..#Xch_Ported_Clm') IS NOT NULL DROP TABLE #Xch_Ported_Clm
	...
	/* last {months} from As_At date */
	DECLARE @as_at_start datetime = DATEADD(m, -26, @as_at)
	
	IF @prd_year_start is null
		SET @prd_year_start = YEAR(@as_at_start)
	
	IF @prd_month_start is null
		SET @prd_month_start = MONTH(@as_at_start)
	...
	/* Clean up remunerations:
		Last {@lag} months
		Before As_At start date
	*/
	SET @SQL = 'DELETE FROM [DART].[dbo].[' + UPPER(@system) + '_RTW]
				WHERE Remuneration_End IN (SELECT DISTINCT TOP ' + CONVERT(varchar, @lag) + ' Remuneration_End
					...
					OR Remuneration_End < ''' + CONVERT(VARCHAR, @as_at_start, 120) + ''''
	EXEC(@SQL)
	...
	/* create #Xch_Ported_Clm table to store XCH claims */
	CREATE TABLE #Xch_Ported_Clm
	(
		claim varchar(19)
	)
	...
	IF UPPER(@system) = 'EML'
	BEGIN
		SET @SQL = 'SELECT DISTINCT Claim_no
					FROM CAD_AUDIT
					WHERE Claims_officer = ''ccxch2015'' or Claims_officer = ''ccgbs2015'''
		
		INSERT INTO #Xch_Ported_Clm
		EXEC(@SQL)
	END
	...
	DECLARE @SQL_Select varchar(MAX) = 'SELECT	pr.Claim_No
												...
												,teed = '
												+ case when UPPER(@system) = 'EML'
															then '(case when xch_cl.claim IS NOT NULL
																			then CONVERT(int, CONVERT(varchar(8), pr.Transaction_date, 112))
																		else wc_Tape_Month
																	end)'
													else 'wc_Tape_Month'
												end +
	...
	DECLARE @SQL_From varchar(MAX) = ' FROM dbo.Payment_Recovery pr INNER JOIN #data_clm cl ON pr.Claim_No = cl.claim'
										+
										case when UPPER(@system) = 'EML'
												then ' LEFT JOIN #Xch_Ported_Clm xch_cl on xch_cl.claim = cl.claim'
											else ''
										end
