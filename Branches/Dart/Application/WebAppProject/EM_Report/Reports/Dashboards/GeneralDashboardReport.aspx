...
<body style="margin: 0;">
    ...
    <script type="text/javascript" language="javascript">
        ...
        function adjustLayout() {
            ...
            //hide group from wow external
            var grpName = $('#grpName', window.parent.document).val();
            if (document.URL.toLowerCase().indexOf('wow') >= 1 && grpName == 'WOWExternal') {
                var ifHeight = $("#ifReport", window.parent.document).height();
                $("#ifReport", window.parent.document).height(ifHeight - 200);
                $("table[alt='level1_table2']").closest('tr').hide();
                $("table[alt='level1_table2']").closest('tr').prev('tr').hide();
            }            
        }

        function doAdditionalWorks() {
            initGraphTitleTooltip();
            removeTooltipWhenHover();
        }

        function removeTooltipWhenHover() {
            $("td[alt^='title_graph_']").each(function () {
                $(this).next().closest('a').css('background-color','red');
                //$(this).closest('a').attr('href', trafficLightTable.find('img').closest('a').attr('href').replace('javascript:void(', ''));
            });
        }
