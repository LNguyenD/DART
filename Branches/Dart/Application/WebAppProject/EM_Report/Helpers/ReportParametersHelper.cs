...
namespace EM_Report.Helpers
{
    public static class ReportParametersHelper
    {
        public static ItemParameter PopulateParametersData(PortfolioReportParameters paramData, out string reportParameterData)
        {
            ...
            parameters = UpdateGeneralParametersData(systemName, parameters);
        ...
        public static ItemParameter PopulateGeneralParametersData(PortfolioReportParameters paramData, out string reportParameterData)
        {
            var itemParameter = new SSRS.ItemParameter();
            var parameters = new List<RS.ItemParameter>();
        ...
        public static IEnumerable<RS.ValidValue> GetValuesList(string systemName, string valueType)
        {
            ...
            if (!string.IsNullOrEmpty(systemName) && !string.IsNullOrEmpty(valueType) && !valueType.Contains("Select ..."))
            {
                ...
                //if(valueTypeEnum != SystemValueType.Portfolio && valueTypeEnum != SystemValueType.EmployerSize)
                //{
                // put Miscellaneous at the end
                //values.Add(new RS.ValidValue { Label = "Miscellaneous", Value = "Miscellaneous" });
                //}
            }
        ...
        public static IEnumerable<RS.ValidValue> GetSubValuesList(string systemName, string value, string valueType)
        {
            ...
            if (!string.IsNullOrEmpty(systemName) && !string.IsNullOrEmpty(value) && !string.IsNullOrEmpty(valueType) && !value.Contains("Select ..."))
            {
                ...
                //if (valueTypeEnum == SystemValueType.Group)
                //{
                //    // put Miscellaneous at the end
                //    values.Add(new RS.ValidValue { Label = "Miscellaneous", Value = "Miscellaneous" });
                //}
        ...
        #endregion Claim officer parameter
        ...
        public static IEnumerable<RS.ValidValue> GetClaimLiabilityIndicators(string systemName)
        {
            ...
            if (!string.IsNullOrEmpty(systemName))
            {
                ...
                {
                    cliamLiabilityIndicators.ForEach(g => values.Add(new RS.ValidValue
                    {
                        Label = g.Description,
                        Value = (systemName != "WOW" ? g.Liability_Id.ToString() : g.Liability_Code)
                    }));
                }
            }
        ...
        #endregion Claim liability indicator parameter

        #region Other parameters

        public static void AddStartDateEndDateParameters(List<RS.ItemParameter> parameters, string startDate, string endDate, string systemName)
        {
            if (string.IsNullOrEmpty(endDate))
            {
                ...
                var tempEndDate = Convert.ToDateTime(repo.GetMaxRptDate(systemName));
        ...
        private static List<RS.ItemParameter> UpdateGeneralParametersData(string systemName, List<RS.ItemParameter> parameters)
        {
            AddStartDateEndDateParameters(parameters, null, null, systemName);
