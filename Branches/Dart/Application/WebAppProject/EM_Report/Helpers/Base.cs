...
namespace EM_Report.Helpers
{
    public class Base
    {
        public static LoginType LoginType
        {
            get
            {
                var loginTypeTemp = Base.GetConfig("LoginType");
                if (string.IsNullOrEmpty(loginTypeTemp))
        ...
        public static string GetSystemNameByUrl(string url)
        {
            ...
            else if (urlUpper.IndexOf("WOW") >= 0)
            {
                systemName = "WOW";
            }
        ...
        public static string GetParameterValueByName(string configUrls, string name)
        {
            if (!string.IsNullOrEmpty(configUrls))
            {
                var parameterItems = configUrls.Split(new char[] { '&' }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in parameterItems)
                {
                    if (item.ToLower().IndexOf(name.ToLower() + "=") >= 0)
                        return item.ToLower().Replace(name.ToLower() + "=", "");
                }
            }
        ...
        public static bool HaveExternalGroupPermission(string systemName, string type, string level)
        {
            return systemName.ToLower() == "wow"
                && type.ToLower() == "group"
                && (level.ToLower() == "2" || level.ToLower() == "3" || level.ToLower() == "4")
                && LoginSession.External_GroupName != string.Empty ? false : true;
        }
    }
}