...
namespace EM_Report.Helpers
{
    public class Login
    {
        ...
        public static void AddLoginCookie(long userid, string username, string password, string domain)
        {
            ...
            Base.AddUpdateCookie(Constants.STR_UserInfoSecurity, Common.Utilities.EnCryption.Encrypt(ipAddress + "|" + pcName + "|" + macAddress + "|" + username + "|" + password + "|" + userid.ToString() + "|" + domain), 365 * 3);
        }
        ...
        public static bool AutoLoginCookie()
        {
            ...
            {
                string decryptCookie = Common.Utilities.EnCryption.Decrypt(userInfo.Value.ToString());
                string[] values = decryptCookie.Split('|');                
                ...
                else
                {
                    ...
                    var response = Login.Authenticate(int.Parse(values[5]), model, values[6]);                     
                    if (response == null)
                    {
                        returnValue = false;
                    }
                    ...
                    {                        
                        Login.LoginAction(response, values[4], true, values[6]);
                        returnValue = true;
                    }
                }
            }
