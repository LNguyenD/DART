    ...
    $('#ReportParameterSection select, #ReportParameterSection input[type="text"]').each(function (index) {
        ...
        if ($(this).prop('type').indexOf('select') > -1) {
            ...
            if (paramValue == 'Yes' || paramValue == 'yes')
                ...
                paramValue = '0';
            else if (paramName == 'Claim_Liability_Indicator') {
                paramValue = $(this).next('div').find(".sbSelector").attr("rel");
            }
        }
        ...
        paramData += "&" + paramName + "=" + (paramValue != '' ? paramValue : 'All');
    });
...
function FixUI() {
    ...
    FixCalendar();
    fixMultiSelectForDropdown("prmClaim_Liability_Indicator");
}
...
//=============== PORTFOLIO REPORT - SUPPORT MULTI OPTIONS ON INPUT PARAMETERS ==============

function fixMultiSelectForDropdown(paramControlId) {
    var txtSelector = $("#" + paramControlId).next('div').find(".sbSelector");

    /* transform Claim liability indicator dropdown to allow user can select multi options */
    $("#" + paramControlId).next('div').find(".sbOptions li").each(function (index) {
        var aItem = $(this).find("a");
        if (aItem != null) {
            $(this).prepend("<div><input type='checkbox' value='" + aItem.attr("rel") + "'><a>" +aItem.text() + "</a></div>");
            aItem.remove();

            // styled
            $(this).find("a").css({ "color": "#848589", "cursor": "pointer", "display" : "inline", "border-bottom" : "none" });
            $(this).find("input[type='checkbox']").css("cursor", "pointer");
            $(this).find("div").css({ "border-bottom": "dotted 1px", "padding" : "7px 0px 7px 3px" });

            //check the checkbox when its parent a is clicked
            $(this).find("a").click(function () {
                var checkBox = $(this).prev("input[type='checkbox']");
                if(checkBox.is(":checked")){
                    checkBox.attr("checked", false);
                    changeTextAfterCheckCheckbox(paramControlId, txtSelector, checkBox);
                }
                else {
            ...
            // register options changed event
            $(this).find("input[type='checkbox']").change(function () {
                var _this = $(this);

                changeTextAfterCheckCheckbox(paramControlId, txtSelector, _this);
            });
        }
    });

    // default: set all of options selected
    setupMultiSelectAll(paramControlId, true);
    txtSelector.attr("rel", "All");
}

function setupMultiSelectAll(paramControlId, isChecked) {
    $("#" + paramControlId).next('div').find(".sbOptions li").each(function (index) {
        var checkboxItem = $(this).find("input[type='checkbox']");
        if (checkboxItem != null)
            checkboxItem.attr("checked", isChecked);
    });
}

function changeTextAfterCheckCheckbox(paramControlId, txtSelector, checkbox) {
    var _this = checkbox;
    // update display text
    if (txtSelector != null) {
        if (_this.attr("value") == 'All') {
            ...
            setupMultiSelectAll(paramControlId, _this.is(":checked"));

            // reset display text to 'All'
            txtSelector.text("All");
            txtSelector.attr("rel", "All");
        }
        else {
            ...
            $("#" + paramControlId).next('div').find(".sbOptions li").each(function (index) {
                var checkboxItem = $(this).find("input[type='checkbox']");
                if (checkboxItem != null && checkboxItem.is(":checked") && checkboxItem.attr("value") != 'All') {
                    selectedItemsCount++;

                    // build values string
                    selectedItems += (selectedItems != "" ? "|" + checkboxItem.attr("value") : checkboxItem.attr("value"));
                }

                if (!_this.is(":checked") && checkboxItem.attr("value") == 'All')
                    checkboxItem.attr("checked", false);
            });

            txtSelector.text(selectedItemsCount == 0 ? "All" : selectedItemsCount + " item(s) selected");
            txtSelector.attr("rel", selectedItems);
        }
    }
}

//=============== END PORTFOLIO REPORT - SUPPORT MULTI OPTIONS ON INPUT PARAMETERS ==============