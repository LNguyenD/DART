...
END
	...
	SELECT	Team = CASE WHEN RTRIM(ISNULL(co.Grp, '')) = '' THEN 'Miscellaneous' ELSE RTRIM(UPPER(co.Grp)) END			
			...
			,NCMM_Actions_This_Week = (case when cada.Claim_Closed_Flag = 'Y'
												...
												dbo.udf_GetNCMMActionsThisWeek(cd.Date_of_Injury, @AsAt_Next_Week_End)
										end)
			,NCMM_Actions_Next_Week = (case when cada.Claim_Closed_Flag = 'Y'
												then ''
											else
												dbo.udf_GetNCMMActionsNextWeek(cd.Date_of_Injury, @AsAt_Next_Week_End)
										end)
			...
			,IsPreClosed = (case when CADA_preclosed.ID is not null
									then 1
								else 0
							end)
			,IsPreOpened = (case when (CADA_preopened.ID is not null OR ISNULL(cada.Date_Claim_Entered, cada.date_claim_received) >= @Start_Date)
									then 1
								else 0
							end)
	FROM	CAD_AUDIT cada
			...
			-- for determining Pre-Opened and Pre-Closed
			LEFT JOIN CAD_AUDIT CADA_preclosed ON CADA_preclosed.Claim_no = cada.Claim_no
				AND CADA_preclosed.Claim_Closed_Flag = 'Y'
				AND CADA_preclosed.ID = (SELECT MAX(ID) FROM CAD_AUDIT CADA2
											WHERE CADA2.Claim_no = CADA_preclosed.Claim_no
												AND CADA2.Transaction_Date < @Start_Date)
			LEFT JOIN CAD_AUDIT CADA_preopened ON CADA_preopened.Claim_no = cada.Claim_no
				AND CADA_preopened.Claim_Closed_Flag = 'N'
				AND CADA_preopened.ID = (SELECT MAX(ID) FROM CAD_AUDIT CADA2
