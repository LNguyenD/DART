...
namespace EM_Report.BLL.Services
{
    ...
    public class UserService : ServiceBase<User, UserDO>, I_UserService
    {
        ...
        protected override IQueryable<User> Filter(string keyword, IQueryable<User> query)
        {
            ...
            //predicate = predicate.Or(p => keyword.IndexOf(p.UserName) > -1);
            //predicate = predicate.Or(p => keyword.IndexOf(p.FirstName) > -1);
            //predicate = predicate.Or(p => keyword.IndexOf(p.LastName) > -1);
            //predicate = predicate.Or(p => keyword.IndexOf(p.Email) > -1);
