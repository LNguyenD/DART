...
BEGIN
	...
	SET @SQL = @SQL + (case when @ClaimType like '%med_only'
							...
							when @ClaimType like '%ncmm_this_week'
								 then ' and [NCMM_Actions_This_Week] <> '''' and [NCMM_Complete_Action_Due] > ''' + CONVERT(VARCHAR, @Reporting_Date, 120) + ''''
							when @ClaimType like '%ncmm_next_week'
								then ' and [NCMM_Actions_Next_Week] <> '''' and [NCMM_Prepare_Action_Due] <= ''' + CONVERT(VARCHAR, DATEADD(week, 3, @Reporting_Date), 120) + ''''
							else '' 
						end)
