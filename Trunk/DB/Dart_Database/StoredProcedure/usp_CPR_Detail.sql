...
BEGIN
	...
	FROM (select *,claim_type='claim_new_all' from #claim_new_all
				...
				union all select *,claim_type='claim_open_ncmm_this_week' from #claim_open_all WHERE NCMM_Actions_This_Week <> '' AND NCMM_Complete_Action_Due > @Reporting_Date
				union all select *,claim_type='claim_open_ncmm_next_week' from #claim_open_all WHERE NCMM_Actions_Next_Week <> '' AND NCMM_Prepare_Action_Due <= DATEADD(week, 3, @Reporting_Date)
	...
	SET @SQL = 'SELECT tmp.Value, Claim_type, tmp.iClaim_Type
					...
					,ncmm_this_week = (select COUNT(distinct Claim_No) from #claim_all where claim_type = tmp.Claim_Type
						and NCMM_Actions_This_Week <> '''' and NCMM_Complete_Action_Due > ''' + CONVERT(VARCHAR, @Reporting_Date, 120) + ''')
					
					,ncmm_next_week = (select COUNT(distinct Claim_No) from #claim_all where claim_type = tmp.Claim_Type
						and NCMM_Actions_Next_Week <> '''' and NCMM_Prepare_Action_Due <= ''' + CONVERT(VARCHAR, DATEADD(week, 3, @Reporting_Date), 120) + ''')
