namespace EM_Report.Service.ServiceImplementations
{
    ...
    public class ActionService : IActionService
    {
        ...
        public UserResponse GetUsers(UserRequest request)
        {
            ...
            else if (request.LoadOptions.Contains(Resource.Get_System_User_by_CronJob))
            {
                response.User = IUserService.GetSystemUserbyCronJob();
            }
            else
            {
                // Validate client tag and access token
        ...
        public PortfolioResponse GetPortfolio(PortfolioRequest request)
        {
            ...
            else if (request.LoadOptions.Contains(Resource.Get_Portfolio_Param_ClaimLiabilityIndicators))
            {
                ...
                {
                    case "HEM":
                        response.ClaimLiabilityIndicators = (IList<string>)IHEMPortfolioService.GetCliamLiabilityIndicators("HEM");
                        break;

                    case "EML":
                        response.ClaimLiabilityIndicators = (IList<string>)IEMLPortfolioService.GetCliamLiabilityIndicators("EML");
                        break;

                    case "TMF":
                        response.ClaimLiabilityIndicators = (IList<string>)ITMFPortfolioService.GetCliamLiabilityIndicators("TMF");
                        break;

                    case "WOW":
                        response.ClaimLiabilityIndicators = (IList<string>)IWOWPortfolioService.GetCliamLiabilityIndicators("WOW");
                        break;
                }
            }
