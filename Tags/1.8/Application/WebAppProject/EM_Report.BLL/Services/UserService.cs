...
namespace EM_Report.BLL.Services
{
    ...
    public class UserService : ServiceBase<User, UserDO>, I_UserService
    {
        ...
        {
            var user = Repository.GetQueryable().Where(l => l.Status == 1 && l.Is_System_User == true && l.Password == "");
            return GetUserFromMapping(user);
        }
        ...
        public User_Ext GetUserExtInfo(string usernameoremail)
        {
            ...
            foreach (DataRow row in data.Rows)
            {
                ...
                //result.SystemList = row["List_Of_SystemId"].ToString() == string.Empty ? (List<Int32>)null : (row["List_Of_SystemId"]).ToString().Split(',').Select(Int32.Parse).ToList();
                result.Is_External_User = (bool)(row["Is_External_User"]);

                var lstSystem = new List<Domain.Systems>();
                //foreach (String st in row["List_Of_System_Group"].ToString() == string.Empty ? null : (row["List_Of_System_Group"]).ToString().Split(','))
        ...
        public int SendmailResetPassword(string email, string resetpasswordurl)
        {
            ...
            if (user != null && user.Is_External_User == true)
            {
                ...
                string urlConfirmation = resetpasswordurl.Remove(resetpasswordurl.LastIndexOf("/") + 1) + "VerifyResetPassword?email=" + email + "&securecheck=" + password;
