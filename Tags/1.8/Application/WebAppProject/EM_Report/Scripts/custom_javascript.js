...
****************************************************************/
$(".dashboardExternalGroupContent .show").click(function () {
    ChangeShowOrgList();
    var isShow = $(".showDash").is(':visible');
    if (isShow) {
        $(".showDash").slideUp('slow');
    }
    else {
        $(".showDash").slideDown('slow');
    }
});
...
function ChangeShowOrgList() {
    var isShow = $(".showDash").is(':visible');
    if (isShow) {
        $(".dashboardExternalGroupContent .show").text("Show Dashboard External Group list");
    }
    else {
        $(".dashboardExternalGroupContent .show").text("Hide Dashboard External Group list");
    }
}
/***************************************************************
VIEW RAW DATA
            ...
            if (id_name == "tab_agencies") {
                ...
                if (level2CrumbtrailsCPR != null) {
                    ...
                    if (systemName == "TMF") {
                        ...
                        // show level 3 lookahead crumtrails for porfolio
                ...
                if (level2CrumbtrailsCPR != null) {
                    if (systemName == "TMF") {
                        ...
                        if (level2CrumbtrailsCPR.indexOf('employer size') >= 0) {
                            ...
                            // show levels 3 lookahead crumtrails for Group
                            $("#lookaheadLevel3CrumbtrailsCPR").show();
                        }
                        else if (level2CrumbtrailsCPR.indexOf('account manager') >= 0) {
                ...
                if (level2CrumbtrailsCPR != null) {
                    if (systemName == "WCNSW") {
                        ...
                        // show level 3 lookahead crumtrails for Account Manager
                        $("#lookaheadLevel3CrumbtrailsCPR").show();
...
function closedPopup(isCPRRaw) {
    ...
    submitTimeInfo(currentTab, isCPRRaw);
    saveStartTime();
}
...
function getCurrentType() {
    ...
    var hasTabs = $(".tab").length > 0;
    if (hasTabs) {
        var hasChild = $(".tab").find("li").attr('id').indexOf('tab') > -1;
        if (hasChild) {
            currentType = getCurrentTabType();
        }
        else {
    ...
    if (currentType.toLowerCase() == 'portfolio' || currentType.toLowerCase() == 'employer_size' || currentType.toLowerCase() == 'division')
        currentType = 'agency';
    else if (currentType.toLowerCase() == 'account_manager' || currentType.toLowerCase() == 'state')
        currentType = 'accountmanager';        
