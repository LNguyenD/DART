...
--------------------------------  
-- D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\UserDefinedFunction\udf_GetNCMMWeeksIn.sql  
--------------------------------  
...
GO--------------------------------  
-- END of D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\UserDefinedFunction\udf_GetNCMMWeeksIn.sql  
--------------------------------  
...
BEGIN
	...
	EXEC [Dart].[dbo].[usp_CPR_Monthly_GenerateData]
END
GO
SET QUOTED_IDENTIFIER OFF
GO
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO [Dart].[dbo].[WOW_Portfolio] EXEC [dbo].[usp_Dashboard_Portfolio_WOW] @System, @AsAt, 1	
		INSERT INTO [Dart].[dbo].[WOW_Portfolio] EXEC [dbo].[usp_Dashboard_Portfolio_WOW] @System, @AsAt, 0
...
--------------------------------  
-- D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\StoredProcedure\usp_Dashboard_Portfolio_WOW.sql  
--------------------------------  
SET ANSI_NULLS OFF
GO
SET QUOTED_IDENTIFIER OFF
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO #claim
			...
			FROM dbo.CLAIM_DETAIL cd									
				INNER JOIN CLAIM_ACTIVITY_DETAIL cada on cada.claim_no = cd.Claim_Number					
			WHERE	CD.is_Null = 0
					AND cada.Claim_Liability_Indicator <> 6
					AND ISNULL(cd.Claim_Number,'') <> ''
					AND (cada.Claim_Closed_Flag <> 'Y' OR cd.Last_Secure_Date >= DATEADD(YY, -1, DATEADD(d, DATEDIFF(d, 0, @AsAt), 0)))
					AND cd.fund not in (98, 99)
	END
	...
	BEGIN
		...
		INSERT INTO #WEEKLY_PAYMENT_ALL
		SELECT  wca_effect.date_claim_received								
				,weeks_paid_old = null
	...
	BEGIN
		...
		SELECT	date_claim_received				
			...
			FROM #WEEKLY_PAYMENT_ALL wpa 
			WHERE date_claim_received >= '2012-10-01'
			GROUP BY date_claim_received					
					,weeks_paid_old
					,incap_week_start
					,incap_week_end
					,incap_week_start_new
	...
	BEGIN
		...
		INSERT INTO #WEEKLY_PAYMENT
		SELECT  case when calc_method = 'OLD'
						then weeks_paid_old
					else 0
				end
				,case when calc_method = 'NEW'
	...
	SELECT	Team = CASE WHEN RTRIM(ISNULL(co.Grp, '')) = '' THEN 'Miscellaneous' ELSE RTRIM(UPPER(co.Grp)) END			
			...
			,CASE WHEN RTRIM(ISNULL(co.Grp, '')) = '' THEN 'Miscellaneous' ELSE RTRIM(UPPER(co.Grp)) END as [group]
			,'' as Division
			,'' as [State]
	FROM	#claim cd						
			INNER JOIN CLAIM_DETAIL cdba on cdba.Claim_Number = cd.claim
				AND cdba.id = (SELECT MAX(cdba2.id)
							  FROM CLAIM_DETAIL cdba2
							  WHERE cdba2.Claim_Number = cdba.Claim_Number)
				AND cdba.is_Null = 0
...
--------------------------------  
-- END of D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\StoredProcedure\usp_Dashboard_Portfolio_WOW.sql  
--------------------------------  
