...
BEGIN
	...
	SET @DataDate = DATEADD(DAY, -1, @AsAt)
			...
			FROM dbo.CLAIM_DETAIL cd LEFT JOIN amendment_exemptions ade on cd.Claim_Number = ade.claim_no 
				...
				AND cada.id = (select MAX(id)
									...
									where cada1.Claim_no = cada.Claim_no
										...
										AND (cada1.Date_Claim_Closed is NULL 
										or cada1.Date_Claim_Closed <= @DataDate)
										AND (cada1.Date_Claim_reopened is NULL
										or cada1.Date_Claim_reopened <= @DataDate))
				AND ISNULL(ptda.id, 1) = ISNULL((SELECT MAX(ptda2.id) 
												FROM ptd_audit ptda2
												WHERE ptda2.policy_no = ptda.policy_no
													  AND ptda2.create_date <= @AsAt
											), 1)
	END
	...
	SELECT	'Group' = CASE WHEN UPPER(@System) = 'TMF'
			...
			,Claim_Liability_Indicator_Group = dbo.udf_GetLiabilityStatusById(
													dbo.udf_ExtractLiabilityStatus(cada.Claim_Liability_Indicator))
