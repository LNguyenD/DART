...
BEGIN
	...
	update #Tem_ClaimDetail set DAYS13 = dbo.udf_NoOfDaysWithoutWeekend(Date_of_Injury, _13WEEKS_) + case when datepart(dw,Date_of_Injury) not IN(1,7) then 1 else 0 end
							...
							,	DAYS13_PRD = dbo.udf_NoOfDaysWithoutWeekend(dbo.udf_MaxDay(Date_of_Injury, @remuneration_Start,'1900/01/01'),dbo.udf_MinDay(DATEADD(dd, -1, _13WEEKS_), @remuneration_End,'2222/01/01'))
											+ case when datepart(dw, dbo.udf_MaxDay(Date_of_Injury, @remuneration_Start, '1900/01/01')) not IN(1,7) then 1 else 0 end
							...
							,	DAYS52_PRD = dbo.udf_NoOfDaysWithoutWeekend(dbo.udf_MaxDay(_26WEEKS_, @remuneration_Start,'1900/01/01'),dbo.udf_MinDay(DATEADD(dd, -1, _52WEEKS_), @remuneration_End,'2222/01/01'))
											+ case when datepart(dw, dbo.udf_MaxDay(_26WEEKS_, @remuneration_Start, '1900/01/01')) not IN(1,7) then 1 else 0 end
							,	DAYS78_PRD = dbo.udf_NoOfDaysWithoutWeekend(dbo.udf_MaxDay(_52WEEKS_, @remuneration_Start,'1900/01/01'),dbo.udf_MinDay(DATEADD(dd, -1, _78WEEKS_), @remuneration_End,'2222/01/01'))
											+ case when datepart(dw, dbo.udf_MaxDay(_52WEEKS_, @remuneration_Start, '1900/01/01')) not IN(1,7) then 1 else 0 end
	...
	select  'Remuneration_Start' = @Transaction_lag_Remuneration_Start
			...
			,Group_ = isnull(UPPER(ctra1.item),'Miscellaneous')
			,Team_ = CASE WHEN rtrim(isnull(coa1.Grp,''))='' then 'Miscellaneous' else rtrim(UPPER(coa1.Grp)) end
			,Case_manager_ = ISNULL ( UPPER(coa1.First_Name+' '+ coa1.Last_Name), 'Miscellaneous')
	...
	from (
			SELECT rtrim(cla.claim_no) as Claim_no
					...
					, case when DAYS13_PRD_CALC < 0 then 0 else round(dbo.udf_MinValue(1, 1.0 * DAYS13_PRD_CALC / nullif(DAYS13_CALC,0)), 10) end as WGT
					,cla.EMPL_SIZE
		...
		LEFT JOIN (SELECT [id],item,value FROM control_audit ctra1 WHERE type = 'GroupLevel') ctra1 ON (CHARINDEX('*'+coa1.grp+'*',ctra1.value)) <> 0 AND ctra1.id = ISNULL((SELECT MAX(ctra2.id) FROM control_audit ctra2
				WHERE ctra2.type = 'GroupLevel'
					AND ((cada1.Claim_Closed_Flag ='Y' AND ctra2.create_date <= dbo.udf_MinDay(ISNULL(cada1.Date_Claim_Closed, cd._13WEEKS_), @Transaction_lag_Remuneration_End, cd._13WEEKS_))
						OR (cada1.Claim_Closed_Flag ='N' AND ctra2.create_date <= dbo.udf_MinDay(cd._13WEEKS_, @Transaction_lag_Remuneration_End, @Transaction_lag_Remuneration_End)))
	...
	from (
			SELECT rtrim(cla.claim_no) as Claim_no
				...
				, case when DAYS26_PRD_CALC < 0 then 0 else round(dbo.udf_MinValue(1, 1.0 * DAYS26_PRD_CALC / nullif(DAYS26_CALC,0)), 10) end as WGT
				,cla.EMPL_SIZE
				,sum(cla.Weeks_Paid_adjusted) as Weeks_Paid
				,_26WEEKS_
		...
		LEFT JOIN (SELECT [id],item,value FROM control_audit ctra1 WHERE type = 'GroupLevel') ctra1 ON (CHARINDEX('*'+coa1.grp+'*',ctra1.value)) <> 0 AND ctra1.id = ISNULL((SELECT MAX(ctra2.id) FROM control_audit ctra2
				WHERE ctra2.type = 'GroupLevel'
					AND ((cada1.Claim_Closed_Flag ='Y' AND ctra2.create_date <= dbo.udf_MinDay(ISNULL(cada1.Date_Claim_Closed, cd._26WEEKS_), @Transaction_lag_Remuneration_End, cd._26WEEKS_))
						OR (cada1.Claim_Closed_Flag ='N' AND ctra2.create_date <= dbo.udf_MinDay(cd._26WEEKS_, @Transaction_lag_Remuneration_End, @Transaction_lag_Remuneration_End)))
					AND ctra2.item = ctra1.item ), '' )
	...
	from (
			SELECT	rtrim(cla.claim_no) as Claim_no	
					...
					, case when DAYS52_PRD_CALC < 0 then 0 else round(dbo.udf_MinValue(1, 1.0 * DAYS52_PRD_CALC / nullif(DAYS52_CALC,0)), 10) end as WGT
					,cla.EMPL_SIZE
					,sum(cla.Weeks_Paid_adjusted) as Weeks_Paid
					,_52WEEKS_
		...
		LEFT JOIN (SELECT [id],item,value FROM control_audit ctra1 WHERE type = 'GroupLevel') ctra1 ON (CHARINDEX('*'+coa1.grp+'*',ctra1.value)) <> 0 AND ctra1.id = ISNULL((SELECT MAX(ctra2.id) FROM control_audit ctra2
				WHERE ctra2.type = 'GroupLevel'
					AND ((cada1.Claim_Closed_Flag ='Y' AND ctra2.create_date <= dbo.udf_MinDay(ISNULL(cada1.Date_Claim_Closed, cd._52WEEKS_), @Transaction_lag_Remuneration_End, cd._52WEEKS_))
						OR (cada1.Claim_Closed_Flag ='N' AND ctra2.create_date <= dbo.udf_MinDay(cd._52WEEKS_, @Transaction_lag_Remuneration_End, @Transaction_lag_Remuneration_End)))
					AND ctra2.item = ctra1.item ), '' )	
	...
	from (
			SELECT	rtrim(cla.claim_no) as Claim_no	
					...
					, case when DAYS78_PRD_CALC < 0 then 0 else round(dbo.udf_MinValue(1, 1.0 * DAYS78_PRD_CALC / nullif(DAYS78_CALC,0)), 10) end as WGT
					,cla.EMPL_SIZE
					,sum(cla.Weeks_Paid_adjusted) as Weeks_Paid
					,_78WEEKS_
			FROM #TEMP_MEASURES cla 
		...
		LEFT JOIN (SELECT [id],item,value FROM control_audit ctra1 WHERE type = 'GroupLevel') ctra1 ON (CHARINDEX('*'+coa1.grp+'*',ctra1.value)) <> 0 AND ctra1.id = ISNULL((SELECT MAX(ctra2.id) FROM control_audit ctra2
				WHERE ctra2.type = 'GroupLevel'
					AND ((cada1.Claim_Closed_Flag ='Y' AND ctra2.create_date <= dbo.udf_MinDay(ISNULL(cada1.Date_Claim_Closed, cd._78WEEKS_), @Transaction_lag_Remuneration_End, cd._78WEEKS_))
						OR (cada1.Claim_Closed_Flag ='N' AND ctra2.create_date <= dbo.udf_MinDay(cd._78WEEKS_, @Transaction_lag_Remuneration_End, @Transaction_lag_Remuneration_End)))
					AND ctra2.item = ctra1.item ), '' )	
	...
	from (
			SELECT	rtrim(cla.claim_no) as Claim_no	
				...
				, case when DAYS104_PRD_CALC < 0 then 0 else round(dbo.udf_MinValue(1, 1.0 * DAYS104_PRD_CALC / nullif(DAYS104_CALC,0)), 10) end as WGT
				,cla.EMPL_SIZE
				,sum(cla.Weeks_Paid_adjusted) as Weeks_Paid
				,_104WEEKS_
			FROM #TEMP_MEASURES cla 
		...
		LEFT JOIN (SELECT [id],item,value FROM control_audit ctra1 WHERE type = 'GroupLevel') ctra1 ON (CHARINDEX('*'+coa1.grp+'*',ctra1.value)) <> 0 AND ctra1.id = ISNULL((SELECT MAX(ctra2.id) FROM control_audit ctra2
				WHERE ctra2.type = 'GroupLevel'
					AND ((cada1.Claim_Closed_Flag ='Y' AND ctra2.create_date <= dbo.udf_MinDay(ISNULL(cada1.Date_Claim_Closed, cd._104WEEKS_), @Transaction_lag_Remuneration_End, cd._104WEEKS_))
						OR (cada1.Claim_Closed_Flag ='N' AND ctra2.create_date <= dbo.udf_MinDay(cd._104WEEKS_, @Transaction_lag_Remuneration_End, @Transaction_lag_Remuneration_End)))
					AND ctra2.item = ctra1.item ), '' )
