namespace EM_Report.Service.ServiceImplementations
{
    ...
    public class ActionService : IActionService
    {
        ...
        /// <summary>
        /// Set Configuration to LoginResponse UserDC
        /// </summary>
        private void SetConfiguration(LoginResponse response)
        {
            response.Configs = new Config()
            {
                ReportServerUrl = Get_AppSetting("ReportServerUrl"),
                ReportServicePath = Get_AppSetting("ReportServicePath"),
                ReportServicePath2010 = Get_AppSetting("ReportServicePath2010"),
                ReportServerDomain = Get_AppSetting("ReportServerDomain"),
                EM_ReportConnectionString = Get_ConnectionString("DartConnectionString"),
                ReportPathPrefix = Get_AppSetting("ReportPathPrefix")
            };
        }
        ...
        public LoginResponse Login(LoginRequest request)
        {
            ...
            var intLogin = IAccountService.Login(_username, request.Password, request.NoLimitLoginAttempts, request.NoDaysBlockedAttempts, request.SystemId, request.LoginType);
            if (intLogin <= 0)
            {
        ...
        public LoginResponse AutoLogin(LoginRequest request)
        {
            ...
            var intLogin = IAccountService.AutoLogin(request.UserId, _username, request.Password, request.NoLimitLoginAttempts, request.NoDaysBlockedAttempts, request.SystemId, request.LoginType);
            if (intLogin <= 0)
            {
                response.Acknowledge = AcknowledgeType.Failure;
                if (intLogin == -1)
        ...
        public External_GroupResponse GetExternal_Groups(External_GroupRequest request)
        {
            ...
            if (request.LoadOptions.Contains(Resource.Get_Option_List))
            {
                var groups = IExternal_GroupService.GetAllQueryable(request.Criteria.SortExpression, request.Criteria.SearchKeyWord);
        ...
        public Dashboard_Graph_DescriptionResponse GetDashboard_Graph_Description(Dashboard_Graph_DescriptionRequest request)
        {
            ...
            {
                var descriptions = IDashboard_Graph_DescriptionService.GetAllQueryable(request.Criteria.SortExpression, request.Criteria.SearchKeyWord);
                ...
                var descriptionPaged = descriptions.ToPagedList<Dashboard_Graph_Description>(request.Criteria.PageIndex, request.Criteria.PageSize);

                response.Dashboard_Graph_Descriptions = request.Criteria.PageSize == 0 && request.Criteria.PageIndex == 0 ? descriptions.ToList() : descriptionPaged.ToList();
            }
