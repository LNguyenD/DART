...
namespace EM_Report.Repositories
{
    ...
    {
        string GetToken();
        LoginResponse Login(string username, string password, int limitLoginAttemptsNumber, int daysBlockedAttemptsNumber, int systemid, LoginType loginType);
        LoginResponse AutoLogin(int userid, string usernamecookie, string passwordcookie, int limitLoginAttemptsNumber, int daysBlockedAttemptsNumber, int systemid, LoginType loginType); 
        bool Logout();
    ...
    public class AccountRepository : RepositoryBase, IAccountRepository
    {
        ...
        public LoginResponse Login(string username, string password, int limitLoginAttemptsNumber, int daysBlockedAttemptsNumber, int systemid, LoginType loginType)
        {
            var request = new LoginRequest().Prepare();
            request.UserName = username;
            request.LoginType = loginType;
            request.Password = password;
            request.NoLimitLoginAttempts = limitLoginAttemptsNumber;
            request.NoDaysBlockedAttempts = daysBlockedAttemptsNumber;
        ...
        /// <param name="password">Password.</param>
        /// <returns>Success or failure flag.</returns>
        public LoginResponse AutoLogin(int userid, string usernameCookie, string passwordcookie, int limitLoginAttemptsNumber, int daysBlockedAttemptsNumber, int systemid, LoginType loginType)
        {
            var request = new LoginRequest().Prepare();
