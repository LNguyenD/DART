...
namespace EM_Report.Controllers
{
    public class Dashboard_TimeAccessController : BaseController
    {
        ...
        public ActionResult Details(string url, string starttime, string endtime)
        {
            ...
            {
                //Get DashboardFrame form Url
                var subUrl = url.Substring(url.LastIndexOf("reports"));
                var DashboardFrameStart = subUrl.IndexOf("/") + 1;
                var DashboardFrameEnd = subUrl.IndexOf("&");
                var DashboardFrame = DashboardFrameEnd > 0 ? subUrl.Substring(DashboardFrameStart, DashboardFrameEnd - DashboardFrameStart) : subUrl.Substring(DashboardFrameStart);

                //Get Dashboard Level Name from Url
                var DashboardLevelNameStart = url.IndexOf("level");
                var DashboardLevelNameEnd = url.IndexOf("?");
                var DashboardLevelName = url.Substring(DashboardLevelNameStart, DashboardLevelNameEnd - DashboardLevelNameStart);

                //Compare to the list of Dashboard and DashboardLevel to get DashboardId and DashboardLevelId
                var DashboardList = DashboardRepository.GetList();
                foreach (var dashboarditem in DashboardList)
                {
                    if (dashboarditem.Url.ToLower().Contains(DashboardFrame))
                    {
                        model.DashboardId = dashboarditem.DashboardId;
                        break;
                    }
                }
                var DashboardLevelList = DashboardLevelRepository.GetList();
                foreach (var dashboardlevelitem in DashboardLevelList)
                {
                    if (dashboardlevelitem.Name.Replace(" ", string.Empty).Trim().ToLower().Contains(DashboardLevelName))
                    {
                        model.DashboardLevelId = dashboardlevelitem.DashboardLevelId;
                        break;
                    }
                }
