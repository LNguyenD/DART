...
BEGIN
	...
	SELECT  tmp.Value,Claim_type
		,overall = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type and [Value]=@Value and [SubValue]=tmp.Value)
						
		,med_only = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=tmp.Value and Is_Medical_Only = 1)
						
		,d_d = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=tmp.Value and Is_D_D = 1)
		...
		,totally_unfit = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=tmp.Value and  Med_Cert_Status = 'TU')
		
		,ffsd_at_work_15_less = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=tmp.Value and  Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek <= 15)
		
		,ffsd_at_work_15_more = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=tmp.Value and  Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek > 15)
						
		,ffsd_not_at_work = (select COUNT(*) from #claim_all	where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=tmp.Value and  Med_Cert_Status = 'SID' and Is_Working = 0)
		
		,pid = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=tmp.Value and  Med_Cert_Status = 'PID')					
		...
		,ncmm_this_week = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type and [Value]=@Value and [SubValue]=tmp.Value and NCMM_Actions_This_Week <> '')
		,ncmm_next_week = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type and [Value]=@Value and [SubValue]=tmp.Value and NCMM_Actions_Next_Week <> '') 
	INTO #total
	FROM
	(
