...
BEGIN
	...
	INTO #claim_new_all 
	FROM [dbo].[udf_PORT_Overall](@System, @Type)
	...
	SELECT  tmp.Value,Claim_type
		,overall = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value)
						
		,med_only = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and Is_Medical_Only = 1)
						
		,d_d = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and Is_D_D = 1)
		...
		,totally_unfit = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and  Med_Cert_Status = 'TU')
		
		,ffsd_at_work_15_less = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and  Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek <= 15)
		
		,ffsd_at_work_15_more = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and  Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek > 15)
						
		,ffsd_not_at_work = (select COUNT(*) from #claim_all	where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and  Med_Cert_Status = 'SID' and Is_Working = 0)
		
		,pid = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type  and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and  Med_Cert_Status = 'PID')					
		...
		,ncmm_this_week = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and NCMM_Actions_This_Week <> '')
		,ncmm_next_week = (select COUNT(*) from #claim_all where claim_type =tmp.Claim_Type and [Value]=@Value and [SubValue]=@SubValue and [SubValue2]=tmp.Value and NCMM_Actions_Next_Week <> '') 
	INTO #total
	FROM
	(
		select * from dbo.uv_PORT_Get_All_Claim_Type
		cross join (select distinct SubValue2 as Value from [dbo].[udf_PORT_Overall](@System, @Type)
