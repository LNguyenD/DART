...
SELECT  top 1000000   
		   ...
		   ,No_Of_Active_Weekly_Claims =
				(SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                    ...
                    WHERE      (UnitType = 'team') AND (Unit = dbo.HEM_AWC.Team) AND (Type = 'Actual'))
           ,Projection =
				...
				/NULLIF((SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                            ...
                            WHERE      (UnitType = 'team') AND (Unit = dbo.HEM_AWC.Team) AND (Type = 'Projection')),0)
FROM         dbo.HEM_AWC
...
SELECT  top 1000000   
		   ...
		   ,No_Of_Active_Weekly_Claims =
				(SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                    FROM          dbo.uv_HEM_AWC_Whole_HEM
                    WHERE      (UnitType = 'am_empl_size') AND (Unit = dbo.HEM_AWC.Empl_Size) AND (Type = 'Actual'))
           ,Projection =
				100*(SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                    FROM          dbo.uv_HEM_AWC_Whole_HEM
                    WHERE      (UnitType = 'am_empl_size') AND (Unit = dbo.HEM_AWC.Empl_Size) AND (Type = 'Actual'))
				/NULLIF((SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                            FROM          dbo.uv_HEM_AWC_Whole_HEM AS uv_HEM_AWC_Whole_HEM_1
                            WHERE      (UnitType = 'am_empl_size') AND (Unit = dbo.HEM_AWC.Empl_Size) AND (Type = 'Projection')),0)
FROM         dbo.HEM_AWC
WHERE     (Time_ID >= DATEADD(mm, - 2,
                          (SELECT     MAX(Time_ID) AS Expr1
                            FROM          dbo.HEM_AWC AS HEM_AWC_1)))
...
SELECT  top 1000000   
		   ...
		   ,No_Of_Active_Weekly_Claims =
				(SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                    FROM          dbo.uv_HEM_AWC_Whole_HEM
                    WHERE      (UnitType = 'portfolio_empl_size') AND (Unit = dbo.HEM_AWC.Empl_Size) AND (Type = 'Actual'))
           ,Projection =
				100*(SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                    FROM          dbo.uv_HEM_AWC_Whole_HEM
                    WHERE      (UnitType = 'portfolio_empl_size') AND (Unit = dbo.HEM_AWC.Empl_Size) AND (Type = 'Actual'))
				/NULLIF((SELECT     SUM(No_of_Active_Weekly_Claims) AS Expr1
                            FROM          dbo.uv_HEM_AWC_Whole_HEM AS uv_HEM_AWC_Whole_HEM_1
                            WHERE      (UnitType = 'portfolio_empl_size') AND (Unit = dbo.HEM_AWC.Empl_Size) AND (Type = 'Projection')),0)
FROM         dbo.HEM_AWC
WHERE     (Time_ID >= DATEADD(mm, - 2,
                          (SELECT     MAX(Time_ID) AS Expr1
                            FROM          dbo.HEM_AWC AS HEM_AWC_1)))
