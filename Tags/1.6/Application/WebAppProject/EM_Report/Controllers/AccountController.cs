...
namespace EM_Report.Controllers
{
    public class AccountController : BaseController
    {
            ...
            else
            {
                ...
                if (Base.LoginSession != null)
                {
                    if (Base.LoginSession.strUserName != null)
                    {
                        ...
                        var response = Login.Authenticate(model, Base.LoginSession.strDomain);
                        if (response != null && response.returnValue > 0)
                        {
                            Login.LoginAction(response, Base.LoginSession.isRememberme, Base.LoginSession.strDomain);
                            returnValue = true;
                        }
                    }
                    else
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Login.LoginAction(response, true, values[3]);
                                returnValue = true;
                            }
                        }
                    }
        ...
        public ActionResult LogIn(LogIn model, string returnUrl)
        {
            ...
            var response = Login.Authenticate(model, domain);
            if (response != null)
            {
                if (response.Acknowledge == AcknowledgeType.Success)
                {
                    Login.LoginAction(response, model.Is_RememberMe, domain);
                    ...
                    {
                        Login.AddLoginCookie(response.User.UserId, model.Username, EnCryption.Encrypt(model.Password), domain);                        
                    }
        ...
        private void FormViewBag()
        {
            if (Base.LoginType == LoginType.Internal)
            {
                ...
                ViewBag.cboDomainList = new SelectList(domainDictionary, "Value", "Key");
            }
        }
    }
}