...
namespace EM_Report.BLL.Services
{
    ...
    public class TMFPortfolioService : ServiceBase<string, TMF_PortfolioDO>, I_TMFPortfolioService
    {
        ...
        public IEnumerable<string> GetSystemAgencies()
        {
            var result = (from p in Repository.GetQueryable()
                          ...
                          where s.AgencyName != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
        ...
        public IEnumerable<string> GetTeams(string group)
        {
            ...
            if (group.Equals("All", StringComparison.OrdinalIgnoreCase))
            {
                result = (from p in Repository.GetQueryable()
                          where p.Team != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
        ...
        public IEnumerable<string> GetSubCategories(string agency)
        {
            ...
            if (agency.Equals("All", StringComparison.OrdinalIgnoreCase))
            {
                result = (from p in Repository.GetQueryable()
                          ...
                          where a.Sub_Category != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
            ...
            else
            {
                result = (from p in Repository.GetQueryable()
                          join a in AgenciesSubCategoryRepository.GetQueryable()
                          on p.Policy_No equals a.POLICY_NO
                          where a.AgencyName.Equals(agency) && a.Sub_Category != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
                          select a.Sub_Category.Trim()).Distinct().OrderBy(s => s).ToList();
            }
        ...
        public IEnumerable<string> GetClaimOfficers(string team)
        {
            ...
            {
                result = (from p in Repository.GetQueryable()
                          where p.Claims_Officer_Name != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
                          select p.Claims_Officer_Name.Trim()).Distinct().OrderBy(c => c).ToList();
            }
            else
            {
                result = (from p in Repository.GetQueryable()
                          where p.Team.Equals(team) && p.Claims_Officer_Name != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
                          select p.Claims_Officer_Name.Trim()).Distinct().OrderBy(c => c).ToList();
            }
        ...
        {
            var result = (from p in Repository.GetQueryable()
                          where p.Claim_Liability_Indicator_Group != "" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
    ...
    public class HEMPortfolioService : ServiceBase<string, HEM_PortfolioDO>, I_HEMPortfolioService
    {
        ...
        {
            var result = (from p in Repository.GetQueryable()
                          where p.Portfolio != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
        ...
        {
            var result = (from p in Repository.GetQueryable()
                          where p.Account_Manager != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
        ...
        public IEnumerable<string> GetEMPL_SizesByAccountManager(string accountManager)
        {
            ...
            {
                result = (from p in Repository.GetQueryable()
                          where p.EMPL_SIZE != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
                          select p.EMPL_SIZE.Trim()).Distinct().OrderBy(e => e).ToList();
            }
            else
            {
                result = (from p in Repository.GetQueryable()
                          where p.Account_Manager.Equals(accountManager) && p.EMPL_SIZE != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
                          select p.EMPL_SIZE.Trim()).Distinct().OrderBy(e => e).ToList();
            }
        ...
        public IEnumerable<string> GetEMPL_SizesByPortfolio(string portfolio)
        {
            ...
            {
                result = (from p in Repository.GetQueryable()
                          where p.Portfolio.Equals(portfolio) && p.EMPL_SIZE != "" && p.Claim_Closed_Flag != "Y" && p.Reporting_Date == Repository.GetQueryable().Max(r => r.Reporting_Date)
                          select p.EMPL_SIZE.Trim()).Distinct().OrderBy(e => e).ToList();
            }
