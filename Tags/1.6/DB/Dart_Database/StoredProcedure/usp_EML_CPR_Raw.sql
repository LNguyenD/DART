...
BEGIN
	...
	BEGIN
		SET @Is_Last_Month = 0
		SELECT top 1 @Reporting_Date = Reporting_Date FROM EML_Portfolio
	...
	BEGIN
		...
		SELECT @Reporting_Date = MAX(Reporting_Date) FROM EML_Portfolio
	END
	...
	BEGIN
		...
		BEGIN
			...
			SELECT top 1 @Reporting_Date_pre = Reporting_Date FROM EML_Portfolio
				WHERE CONVERT(datetime, Reporting_Date, 101) 
			...
			-- Return data
			SELECT * FROM [dbo].[uv_PORT] cpr WHERE [System] = 'EML'
				AND ISNULL(Is_Last_Month, 0) = @Is_Last_Month
				AND Reporting_Date = @Reporting_Date
