...
BEGIN
	...
	BEGIN
		SET @Is_Last_Month = 0
		SELECT top 1 @Reporting_Date = Reporting_Date FROM TMF_Portfolio
	...
	BEGIN
		...
		SELECT @Reporting_Date = MAX(Reporting_Date) FROM TMF_Portfolio
	END
	...
	BEGIN
		...
		BEGIN
			...
			SELECT top 1 @Reporting_Date_pre = Reporting_Date FROM TMF_Portfolio
				WHERE CONVERT(datetime, Reporting_Date, 101) 
			...
			-- Return data
			SELECT * FROM [dbo].[uv_PORT] cpr WHERE [System] = 'TMF'
				AND ISNULL(Is_Last_Month, 0) = @Is_Last_Month
				AND Reporting_Date = @Reporting_Date
