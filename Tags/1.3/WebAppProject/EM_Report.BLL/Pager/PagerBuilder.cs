    ...
    internal class PagerBuilder
    {
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<span class=\"Button\"><a  class=\"Disable\">{0}</a></span>", item.Text));
            ...
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? _html.Encode(item.Text)
                                                 : String.Format("<span class=\"Button \"><a href='{0}'>{1}</a></span>", url, item.Text));
        ...
        private MvcHtmlString GenerateJqAjaxPagerElement(PagerItem item)
        {
            if (item.Disabled)
                return CreateWrappedPagerElement(item, String.Format("<span class=\"Button\"><a class=\"Disable\">{0}</a></span>", item.Text));
                    //String.Format("<a disabled=\"disabled\">{0}</a>", item.Text));
            return CreateWrappedPagerElement(item, String.Format("<span class=\"Button \">{0}</span>", GenerateAnchor(item)));
                //GenerateAnchor(item));
        }
