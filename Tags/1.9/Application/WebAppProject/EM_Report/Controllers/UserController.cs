    ...
    public class UserController : BaseController
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            else
            {
                ...
                foreach (var s in UserRepository.GetSystemList())
                {
                    ...
                    if(!model.External_GroupIdList.IsNullOrEmpty())
                    {
                        ViewData["cboExternal_Group_" + s.Name] = model.External_GroupIdList[s.Name];
                    }
                }
                FormViewBag(model);
                return View(model);
        ...
        public ActionResult Details(User model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    // clear fake password
                    if (model.Password == "fakePassword")
                        model.Password = "";
                    ...
                    if (model.UserId > 0)
                    {
                        ...
                        {
                            Dictionary<string, int> lstExternal_Group_Id = new Dictionary<string, int>();
                            var Is_Selected_One_External_Group = false;
                            var lstSystem = UserRepository.GetSystemList();
                            foreach (var s in lstSystem)
                            {
                                if (Request.Form["cboExternal_Group_" + s.Name] != "0")
                                {
                                    Is_Selected_One_External_Group = true;                                    
                                }
                                
                            }
                            if (!Is_Selected_One_External_Group)
                            {
                            ...
                            {   
                                foreach (var s in lstSystem)
                                {
                                    lstExternal_Group_Id.Add(s.Name, int.Parse(Request.Form["cboExternal_Group_" + s.Name]));
                                }                                                      
                            }                            
                            model.External_GroupIdList = lstExternal_Group_Id;
                            model.Is_System_User = false;
                        ...
                        else if(Request["Is_System_User"] == "True")
                        {
                            model.Is_System_User = true;
                            UserRepository.Update(model);
                            ShowSuccess(Resource.msgSaveSuccess);
                        }
                        else
                    ...
                    else
                    {
                        ...
                        foreach (var s in lstSystem)
                        {
                            ...
                            if (Request.Form["cboExternal_Group_" + s.Name] != "0")
                            {
                                ViewData["cboExternal_Group_" + s.Name] = int.Parse(Request.Form["cboExternal_Group_" + s.Name]);
                                Is_Selected_One_External_Group = true;
                                usrName = s.Name + @"\" + model.UserName;
                            }
                        }
        ...
        {
            ViewBag.External_Groups = External_GroupRepository.GetList("", "", GroupStatusFilter.Active);
