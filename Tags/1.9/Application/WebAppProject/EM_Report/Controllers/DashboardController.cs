        ...
        public ActionResult Level0()
        {
            try
            {
                ...
                if (Base.LoginSession.isExternal)
                {
                    ...
                    if (!havePermissionEx && Base.LoginSession.strUserName.ToLower().Contains("wow"))
                    {
                        if(Base.LoginSession.External_GroupName != "")
                            return Redirect(Base.AbsoluteUrl("~/dashboard/level2?Type=state&reportpath=/emreporting/reports/WOW_CPR_Week_Month_Summary"));
                        else
                            return Redirect(Base.AbsoluteUrl("~/dashboard/level2?Type=group&reportpath=/emreporting/reports/WOW_CPR_Week_Month_Summary"));
                    }
                    else
                    {
