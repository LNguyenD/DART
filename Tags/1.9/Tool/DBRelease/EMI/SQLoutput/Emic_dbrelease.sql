...
--------------------------------  
-- D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\UserDefinedFunction\udf_GetNCCMPrepareActionDueDate.sql  
--------------------------------  
...
GO--------------------------------  
-- END of D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\UserDefinedFunction\udf_GetNCCMPrepareActionDueDate.sql  
--------------------------------  
...
--------------------------------  
-- D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\UserDefinedFunction\udf_NoOfWeeks.sql  
--------------------------------  
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].udf_NoOfWeeks') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
	DROP FUNCTION [dbo].udf_NoOfWeeks
GO

/****** Object:  UserDefinedFunction [dbo].udf_NoOfWeeks    Script Date: 04/23/2015 11:08:20 ******/
SET ANSI_NULLS OFF
GO
...
CREATE FUNCTION [dbo].udf_NoOfWeeks(@StartDate DATETIME, @EndDate DATETIME)
	RETURNS INT
AS
BEGIN
...
GRANT  EXECUTE  ON [dbo].udf_NoOfWeeks TO [EMICS]
GRANT  EXECUTE  ON [dbo].udf_NoOfWeeks TO [EMIUS]
GRANT  EXECUTE  ON [dbo].udf_NoOfWeeks TO [DART_Role]
GO--------------------------------  
-- END of D:\Work\Project\Dart\Branches\Dart\Tool\DBRelease\EMI\tmpChange\UserDefinedFunction\udf_NoOfWeeks.sql  
--------------------------------  
---------------------------------------------------------- 
------------------- View 
---------------------------------------------------------- 
...
begin	
	...
	declare @Reporting_Date_End datetime, @Last3Month_Start datetime, @VarDate datetime
...
begin		
	...
	declare @Reporting_Date_End datetime, @Last3Month_Start datetime
		, @Last3Year_Start datetime, @VarDate datetime
...
BEGIN
	...
	DECLARE @LastYear int = YEAR(DATEADD(m, -12, GETDATE()))
	DECLARE @LastYearMonth int = MONTH(GETDATE())
...
BEGIN
	...
	SELECT	Team = CASE WHEN RTRIM(ISNULL(co.Grp, '')) = '' THEN 'Miscellaneous' ELSE RTRIM(UPPER(co.Grp)) END			
			...
			,NCMM_Actions_This_Week = (case when cd.Claim_Closed_Flag = 'Y'
												...
												dbo.udf_GetNCMMActionsThisWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week))
										end)
			,NCMM_Actions_Next_Week = (case when cd.Claim_Closed_Flag = 'Y'
												then ''
											else
												dbo.udf_GetNCMMActionsNextWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week))
										end)
			...
			,Action_Required = case when dbo.udf_GetNCMMActionsThisWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)) <> ''
										or dbo.udf_GetNCMMActionsNextWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)) <> ''
										then 'Y'
									else 'N'
							   end
			...
			,Weeks_In = dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)
			,Weeks_Band = case when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 0 and 12 then 'A.0-12 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 13 and 18 then 'B.13-18 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 19 and 22 then 'C.19-22 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 23 and 26 then 'D.23-26 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 27 and 34 then 'E.27-34 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 35 and 48 then 'F.35-48 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 48 and 52 then 'G.48-52 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 53 and 60 then 'H.53-60 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 61 and 76 then 'I.61-76 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 77 and 90 then 'J.77-90 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 91 and 100 then 'K.91-100 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 101 and 117 then 'L.101-117 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) between 118 and 130 then 'M.118-130 WK'
							   when dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week) > 130 then 'N.130+ WK'
						  end
			...
			-- NCMM Actions
			,[NCMM_Complete_Action_Due] = case when dbo.udf_GetNCMMActionsThisWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)) <> ''
											then DATEADD(week
														,dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)
														,ISNULL(cd.date_claim_received,cd.date_claim_entered))
										end
			,[NCMM_Complete_Remaining_Days] = case when dbo.udf_GetNCMMActionsThisWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)) <> ''
												then dbo.udf_NoOfWorkingDaysV2(@Reporting_Date
																				,DATEADD(week
																						,dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)
																						,ISNULL(cd.date_claim_received,cd.date_claim_entered)))
											end
			,[NCMM_Prepare_Action_Due] = case when dbo.udf_GetNCMMActionsNextWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)) <> ''
												then dbo.udf_GetNCCMPrepareActionDueDate(
														dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)
														,ISNULL(cd.date_claim_received,cd.date_claim_entered)
													)
										end
			,[NCMM_Prepare_Remaining_Days] = case when dbo.udf_GetNCMMActionsNextWeek(dbo.udf_NoOfWeeks(ISNULL(cd.date_claim_received,cd.date_claim_entered),@AsAt_Next_Week)) <> ''
													then dbo.udf_NoOfWorkingDaysV2(@Reporting_Date
