...
BEGIN	
	...
	--- WOW ---
	IF NOT EXISTS (SELECT name FROM sys.indexes  WHERE name = N'idx_WOW_Portfolio_Claim_Liability_Indicator_Group')
	BEGIN
		CREATE NONCLUSTERED INDEX [idx_WOW_Portfolio_Claim_Liability_Indicator_Group] ON [dbo].[WOW_Portfolio]
		(
			[Claim_Liability_Indicator_Group] ASC
		)WITH (SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF) ON [PRIMARY]
	END
	
	IF NOT EXISTS (SELECT name FROM sys.indexes  WHERE name = N'idx_WOW_Portfolio_Reporting_Date')
	BEGIN
		CREATE NONCLUSTERED INDEX [idx_WOW_Portfolio_Reporting_Date] ON [dbo].[WOW_Portfolio]
		(
			[Reporting_Date] ASC
		)WITH (SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF) ON [PRIMARY]
	END
	
	IF NOT EXISTS (SELECT name FROM sys.indexes  WHERE name = N'idx_WOW_Portfolio_RAW_Data')
	BEGIN
		CREATE NONCLUSTERED INDEX [idx_WOW_Portfolio_RAW_Data] ON [dbo].[WOW_Portfolio]
		(
			[Reporting_Date] ASC,
			[Med_Cert_Status_This_Week] ASC,
			[Id] ASC,
		...
		[ClaimStatus]) WITH (SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF) ON [PRIMARY]
	END
