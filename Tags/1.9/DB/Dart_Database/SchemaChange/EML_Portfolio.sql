...
ELSE
	...
	--BROKER
	IF COL_LENGTH('EML_Portfolio','Broker_Name') IS NULL
	BEGIN	
		ALTER TABLE [dbo].[EML_Portfolio]
		ADD [Broker_Name] [varchar] (256) NULL
	END
	
	IF COL_LENGTH('EML_Portfolio','Broker_No') IS NULL
	BEGIN	
		ALTER TABLE [dbo].[EML_Portfolio]
...
/* Update Broker_Name, Broker_No in EML_Portfolio table */
UPDATE eml
SET Broker_Name = RTRIM(LTRIM((brk.NAME)))
	,Broker_No = brk.BROKER_NO
FROM EML_Portfolio eml
	LEFT JOIN (SELECT br.BROKER_NO, br.NAME, Claim_Number
						FROM [emiwebrep].[dbo].CLAIM_DETAIL cld
							LEFT JOIN [emiwebrep].[dbo].POLICY_TERM_DETAIL PTD on cld.Policy_No = ptd.POLICY_NO
							LEFT JOIN [emiwebrep].[dbo].Broker br on PTD.Broker_No = Br.Broker_no) as brk
		ON eml.Claim_No COLLATE SQL_Latin1_General_CP1_CI_AS = brk.Claim_Number