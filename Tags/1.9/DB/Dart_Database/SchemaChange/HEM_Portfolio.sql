...
ELSE
	...
	--BROKER
	IF COL_LENGTH('HEM_Portfolio','Broker_Name') IS NULL
	BEGIN	
		ALTER TABLE [dbo].[HEM_Portfolio]
		ADD [Broker_Name] [varchar] (256) NULL
	END
	
	IF COL_LENGTH('HEM_Portfolio','Broker_No') IS NULL
	BEGIN	
		ALTER TABLE [dbo].[HEM_Portfolio]
...
/* Update Broker_Name, Broker_No in HEM_Portfolio table */
UPDATE hem
SET Broker_Name = RTRIM(LTRIM((brk.NAME)))
	,Broker_No = brk.BROKER_NO
FROM HEM_Portfolio hem
	LEFT JOIN (SELECT br.BROKER_NO, br.NAME, Claim_Number
						FROM [hemwebrep].[dbo].CLAIM_DETAIL cld
							LEFT JOIN [hemwebrep].[dbo].POLICY_TERM_DETAIL PTD on cld.Policy_No = ptd.POLICY_NO
							LEFT JOIN [hemwebrep].[dbo].Broker br on PTD.Broker_No = Br.Broker_no) as brk
		ON hem.Claim_No COLLATE SQL_Latin1_General_CP1_CI_AS = brk.Claim_Number