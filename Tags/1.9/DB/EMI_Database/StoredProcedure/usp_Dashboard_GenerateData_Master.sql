...
BEGIN
	...
	DECLARE @start_period_year int = 2012
	...
	IF UPPER(@System) = 'TMF'
		BEGIN
			...
			BEGIN				
				EXEC usp_Dashboard_RTW_GenerateData 'TMF', @start_period_year, @start_period_month
	...
	ELSE IF UPPER(@System) = 'EML' 
		BEGIN
			...
			IF DATEDIFF(DD,@firstDateOfMonth,GETDATE()) >= 9
			BEGIN				
				EXEC usp_Dashboard_RTW_GenerateData 'EML', @start_period_year, @start_period_month
	...
	ELSE
		BEGIN
			...
			IF DATEDIFF(DD,@firstDateOfMonth,GETDATE()) >= 9
			BEGIN				
				EXEC usp_Dashboard_RTW_GenerateData 'HEM', @start_period_year, @start_period_month
